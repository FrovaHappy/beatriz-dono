...
Lo primero es clonar o hacer fork del repositorio, si lo que quieres es colaborar puedes leer el [/about/contributing](/about/contributing) para más información.
...
<Aside type="note">
  Si estás usando Docker, solo necesitas ejecutar el paso 2 y 3 para configurar las variables de entorno y ejecutar el comando `docker:dev` para iniciar el servidor en local.
</Aside>
...
PORT = '3000'
PUBLIC_URL_OATH_CALLBACK = 'http://localhost:3000/oauth/callback'
...
- **PUBLIC_URL_OATH_CALLBACK**: url de la aplicación para la autenticación de discord, configura la aplicación en `OAuth2`, luego asigna la url de retorno en `Redirect URIs` en este caso es `http://localhost:3000/oauth/discord`
- **PUBLIC_URL_API**: url de la aplicación para el api
- **PUBLIC_URL_JOIN_BOT**:
  url de la aplicación para el bot de discord, puedes usar la herramienta que te facilita el portal
  accede a la sección de `OAuth2`, luego a `URL Generator`, en el scopes selecciona `bot`, luego en los permisos selecciona `administrator`  (o lo que quieras) y copia la url
